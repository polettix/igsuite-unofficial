...
sub ParseLogRow
 {
  $Hylafax::hylafax_version ||= GetVersion();

  if ( $Hylafax::hylafax_version =~ /(\d+)\.(\d+)/ && $1 < 6 )
   {
    ## log format before Hylafax 6.0
  ...
  else
   {
    ...
    #Roby 20100526
    #Le due righe sotto sono state sostituite con quelle successive
    #Il sesto campo ora contiene anche il path del file .tif in caso di RECV
    #quindi puo' essere alfanumerico.
    #\t
    #(\d*)                 #6 jobid: The job number for outbound calls.
                    
    \t
    ([^\t]+)              #6 jobid: The device identifier for the modem
    ...
    #Roby 20100526
    #Corretta la specifica del formato
    \t
    (\d+\:\d+:\d+)        #13 jobtime: The duration of the session; in the
                          #   format HH:MM:SS. This time includes setup overhead
                          #   and any time spent cleaning up after a call.
    ...
    #Corretta la specifica del formato
    \t
    (\d+:\d+:\d+)         #14 conntime: The time spent on the phone; in the
                          #   format HH:MM:SS. This should be the time used by
                          #   the PTT to calculate usage charges.
...
sub ConnectFax
 {
  ...
  ## Get Hylafax Version
  $hylafax_version = $ftp->message();
  $hylafax_version =~ s/.+HylaFAX.+(\d+\.\d+).+/$1/i;
...
sub GetVersion
 {
  ...
  die("Can't retrieve Hylafax Version!\n") if !$hylafax_version;
  return $hylafax_version;
 }
...
sub PrintFaxStat
 {
  ...
  my $msg = "Hylafax $Hylafax::hylafax_version - ".
            "Server: $host - ".
            "$IG::lang{user}: $login<br>";

  for ( $ftp->dir('status') ) 
   {
    if ($_=~ /Modem/)
     {
      $modem{$1} = "$2 ($1)" if $_=~ s/Modem (\w+) \((.+)\)/Modem Fax in $1 \($2\)/g;
