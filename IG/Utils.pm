...
sub MkMapLink
 {
  ...
  my $link = "http://maps.google.$dom/maps?".
	"f=q&amp;".
...
Parse an html document and return $document and @unresolved.
  ...
  for my $i ( 1 .. IG::GetDaysInMonth( $data{month}, $data{year} ) )
   {
    while (1)
	 ...
	 { $style .= "background: $IG::clr{bg_link}\"" }
        ...
        $daylink =~ s/DAYOFTHEWEEK/$lang{"day_$day_of_the_week"}/g;
