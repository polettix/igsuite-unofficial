...
sub SendMsg
 {
  ...
  if ( !$smtp_server && $IG::ext_app{sendmail} && -e $IG::ext_app{sendmail} )
   {
    ## In some cases user uses only CC or BCC to hide destination message
    $mail{To} = $mail{From} if !$mail{To};
...
sub EmptyTrash
 {
  my %data = @_;
     $data{user} ||= $IG::auth_user;
  my $deleted_msg;
  my $query = ( $data{user} eq 'all' && IG::CheckPrivilege('sys_user_admin')
                ? ''
                : "owner='".DbQuote($data{user})."' and " ).
              "folder='TRASH'";

  ## start progress indicator
  require IG::ProgressIndicator;
  my $pi = IG::ProgressIndicator->new( title => $lang{emptytrash} );
     $pi->start() or return;

  ## count how many messages there are in TRASH
  my $cid = DbQuery( type  => 'AUTO',
                     query => "select count(*) ".
                              "from email_msgs ".
                              "where $query" );
  my $msgs_in_trash = FetchRow( $cid );

  ## select messages to delete from db and filesystem
  my $cid = DbQuery( type  => 'AUTO',
                     query => "select id, owner ".
                              "from email_msgs ".
                              "where $query" );
  while ( my ( $id, $user ) = FetchRow( $cid ) )
   {
    ## Delete message on filesystem
    ++$deleted_msg;
    my $msg_to_delete = IG::UserDir( $user ) . ${IG::S} .
                        'MailBox'            . ${IG::S} .
                        'TRASH'              . ${IG::S} . $id;
    ...
    DbQuery( query => [("delete from email_msgtags ".
	                ...
	                " and referenceid='".DbQuote($user.'_'.$id)."'"
		       )],
             type  => 'UNNESTED' );

    $pi->update( content => "<br>$lang{message}: $deleted_msg / $msgs_in_trash<br>",
                 status_bar => { perc => int( $deleted_msg * 100 / $msgs_in_trash ) } );
   }

  DbQuery( query => "delete from email_msgs where $query",
           type  => 'UNNESTED' );

  LogD("email trash emptied for '$data{user}'", 'delete', 'email_msgs');

  $pi->end
   ( on_success      => $lang{trash_emptied},
     button_close    => 'true',
     button_continue => 'true',
     status_bar      => { perc => 100 },
     continue_url    => 'webmail?action=displayheaders&amp;folder=TRASH' );
...
sub MkLink
 {
  ...
  ## Net protocols. 
  my $delim = "\\s\\\"\\'\\;\\,\\r\\n<>\\(\\)";
  $text=~ s/([$delim]|^)(https|http|ftp)(\:\/\/)([^$delim]+)([$delim]|$)/
            $1 . IG::BuildLink("$2$3$4") . $5/egi;
  ...
  foreach my $img_name (keys %images)
   {
    ...
    my $img_url = $images{$img_name} =~ /^http\:\/\//i
                ? $images{$img_name}
                : $images{$img_name} =~ /^\//
...
=for
  ...
  Tag handling/caching.
  webmail have to call ResetTag at the beginning of each request.
  TagAdded has to be called each time a never used tag may be added (not when
  a tag already in list is added: action/autotag).
  TagRemoved has to be called each time a tag is removed from a mail.
...
sub GetTags
 {
  if( $on{folder} eq 'PROTOCOLS' ) 
   {
    my @all_tags = ();
    my $cid = DbQuery( query => "select email_msgtags.name ".
                                "from email_msgtags ".
		                "group by email_msgtags.name ".
				"order by email_msgtags.name",
                       type  => 'UNNESTED' );
    while ($_ = FetchRow($cid)) { push @all_tags,  $_} ;
    return \@all_tags;
   }
   
  return \@email_tags if @email_tags;
 
  my $filename = __user_tag_filename();
  readfile: if( -e $filename ) 
   {
    open(TAGS,'<',$filename) || last readfile;
    while( <TAGS> ) 
     {
      chomp;
      push @email_tags, $_;
  ...
  (push @email_tags,  $_) while $_ = FetchRow($cid);
  
  writefile: 
   {
    open(TAGS,'>',$filename) || last writefile;
    print TAGS "$_\n" for @email_tags;
    close TAGS;
   }
...
sub TagAdded
 {
  ...
  push @email_tags, $tag if $#email_tags >= 0;
  unlink __user_tag_filename();
 }
...
sub __user_tag_filename
 {
  return IG::UserDir($auth_user).$IG::S.'email_tags';
 }
