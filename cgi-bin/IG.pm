...
use vars qw(	%privileges	%timezones	%countries	$www_user
		...
		$folderquota	$attlimit	$users_allow    $homedirspoolname
...
@EXPORT = qw(	HtmlHead	HtmlFoot	TaskHead	TaskFoot
		...
		MkUrl DeUrl Qs	Br		MkEntities	QuoteParams
  ...
  $SIG{__DIE__} =
   sub {
	...
	print qq~<tr><td colspan=2 bgcolor="#EEEEEE">
		...
		Administrator or if you want, send this message to
		staff\@igsuite.org and help us to improve IGSuite
		</td></tr></table>
		</td></tr></table>
		</body></html>~ if $req;
...
##
## LAST ID USED: 137
## REMOVED: sys_tickler_view',		[47, 'tickler'],
...
%privileges = (
	...
	'equipments_report',            [80, 'equipments'],
  ...
  if ( _valid_session() )
  ...
  elsif ( $request_method ne 'commandline' && $cgi_name ne 'mkstruct.pl')
   {
    ...
    die("Error! - IGSuite can't verify client IP or host name. ".
        "IGSuite need it! Try to check your DNS server ".
        "or your 'hosts' configuration file.\n") if !$remote_host;
...
Get a query string you can use in a Url address
...
sub Qs
 {
  ...
  return substr( $qs, 0, length($qs)-5 );
 }
...
=head3 NormalizeNewline($Text, $Nl)
...
sub GetNiceDate
 {
  ...
  return   $today_diff == 0  ? $lang{today}
         ...
         : (    $today_diff < -1
             && $today_diff > -7 ) ? $lang{'day_'. GetDayByDate($day,
                                                                $month,
                                                                $year) }
...
sub CkDate
 {
  my ($data, $blank) = (@_);
  my ($ck_date_d, $ck_date_m, $ck_date_y);

  ## trim end spaces or separators
  $data =~ s/[\s\.\-\/]+$//;
  ...
  if ( $data =~ /^[\-\+ ]\d+[dmy]$/i )
   {
    ## calculated date
    $data =~ s/^ //;
    ($ck_date_d, $ck_date_m, $ck_date_y) = SumDate(0,0,0,$data);
   }
   ...
   {
    ## yyyy.mm.dd
    ($ck_date_y, $ck_date_m, $ck_date_d) = ($1, $2, $3);
   }
        ...
        )
   {
    if ($date_format eq 'European' || $date_format eq 'German')
     {
      ## dd.mm.yyyy
      ($ck_date_d, $ck_date_m, $ck_date_y) = ($1,$2,$3);
     }
    elsif ($date_format eq 'Sql')
     {
      ## mm.dd.yyyy
      ($ck_date_d, $ck_date_m, $ck_date_y) = ($2,$1,$3);
     }
   }
  else
   { return; }
  ...
  return if    $ck_date_y < 1900
            ...
            || $ck_date_m > 12
            || $ck_date_d < 1
            || $ck_date_d > GetDaysInMonth($ck_date_m, $ck_date_y);

  return ( GetDateByFormat($ck_date_d, $ck_date_m, $ck_date_y) );
 }
  ...
  if ( $on{print} && !$executed{redirect} )
   {
    ...
    ## strip http head
    $prout_page =~ s/.*(^<\!DOCTYPE html.+)/$1/smi;
  ...
  $html .= "// Form Autofocus\n".
           ( $form{focusthis}
             ...
             : $form{autofocus} eq 'true'
               ? "Event.observe(window, \"load\", function()".
                 " { if(document.forms[0]) ". #XXX2IMPROVE
...
sub TaskHead
 {
  ...
  ## IG Logo (IGBox)
  $html .= ( $data{icon}==5 || $tema eq 'microview_' || $data{igbox} eq 'false'
  ...
  if ($data{icon}==5)
   ...
   {
    ## Attach icon
    $html .= Img( href   => Qs( 'postit',
                                { action    => 'proto',
                                  type      => 'Attach',
                                  sharemode => 1,
                                  category  => $lang{$cgi_name},
                                  link      => "$cgi_url/$cgi_name".
                                               "$ENV{PATH_INFO}?".
                                               $query_string,
                                  title     => $page_title } ),
                  target => 'mainf',
  ...
  elsif ( $ico==4 )
   {
    $html.="<table style=\"$style_size\" cellpadding=0 cellspacing=0>
	    <tr><td style=\"width:7px; height:7px;\">
	         <img alt=\"lt\" src=\"$IG::img_url/msglefttop.gif\" class=\"noprint\" width=7 height=7></td>
		<td style=\"width:100%; height:7px; background-image: url($IG::img_url/msgtop.gif)\">
		 <img alt=\"t\" src=\"$IG::img_url/msgtop.gif\" class=\"noprint\" width=1 height=7></td>
		<td style=\"width:17px; height:7px;\">
		 <img alt=\"tr\" src=\"$IG::img_url/msgtopright.gif\" class=\"noprint\" width=10 height=7></td>
	    </tr>";

    $html.="<tr><td style=\"width:7px; height:100%; background-image: url($IG::img_url/msgleft.gif)\">
                 <img alt=\"l\" src=\"$IG::img_url/msgleft.gif\" class=\"noprint\" width=7 height=1></td>
 		<td style=\"background-color:#f9f9f9\">$msg</td>
		<td style=\"width:10px; height:100%; background-image: url($IG::img_url/msgright.gif)\">
		 <img alt=\"r\" src=\"$IG::img_url/msgright.gif\" class=\"noprint\" width=10 height=1></td>
	    </tr>";

    $html.="<tr><td style=\"width:7px; height:16px;\">
                 <img alt=\"lb\" src=\"$IG::img_url/msgleftbottom.gif\" class=\"noprint\" width=7 height=16></td>
		<td style=\"width:8px; height:16px; background-image: url($IG::img_url/msgbottom.gif)\">
		 <img alt=\"b\" src=\"$IG::img_url/msgbottom.gif\" class=\"noprint\" width=8 height=16></td>
		<td style=\"width:7px; height:16px;\">
		 <img alt=\"br\" src=\"$IG::img_url/msgbottomright.gif\" class=\"noprint\" width=10 height=16></td>
	    </tr></table>\n";
   }
  elsif ( $ico==5 )
   {
...
sub QuickHelp
 {
  ...
  $html = $data{href}
        ? "<a id=\"$data{id}\" $data{target}href=\"$data{href}\"$html</a>\n"
...
sub TaskListMenu
 {
  ...
  $task_list_cols = 0;
  $task_list_rows = 0;
  $on{sortdirection} ||= $list_order || 'desc';
  my $sortdirection = $on{sortdirection} eq 'desc' ? 'asc' : 'desc';
 
  $html = "\n<table class=\"tasklist\" cellspacing=1><thead><tr>";
  foreach (@item)
   {
    ...
    if ( $on{order} && @$_[1] =~ /order\=${on{order}}[^\w]*/ )
     {
      $html .= Img( href  => "$cgi_name?".
                    ...
                    src   => "$IG::img_url/freccia${sortdirection}.gif",
                    width => 7,
...
sub MkProtocolList
 {
  ...
  my $cid = DbQuery( query => "select id, name from reports ".
                              ...
                              "order by name",
                     type  => 'UNNESTED' );
  push @personal_reports, [$reports_rows[0],
                           '   '.
                           $reports_rows[1]] while @reports_rows = FetchRow($cid);

  ## set a view per page
  my $base_query = "SELECT count(*) FROM $data{table} ".
                   ( $query =~ /contacts\./
                     ? "left join contacts ".
                       "on contacts.contactid=$data{table}.contactid "
                     : '' ).
                   ( $query =~ /contracts_phases\./
...
sub MkTaskPaging
 {
  ...
  if ($tema ne 'printable_')
   ...
   {
    $html = $on{pos} eq 'all' ? '' : "$lang{page} $on{pos} / $page_cnt";
   }
...
sub MkCgiEnv
 { 
  ...
  if ( !$request_method || @ARGV || $request_method eq 'commandline' )
   {
  ...
  if ( $on{igsuiteid} )
   {
    $set_cookie{igsuiteid} =
    $cookie{igsuiteid}     = $on{igsuiteid};
   }
  ...
  if ( $ENV{QUERY_STRING} )
   {
    foreach my $key (keys %on)
     {
      next if $key =~ /^(attach|formid|list2xls|print)$/;
      my @values = ref($on{$key}) eq 'ARRAY' ? @{$on{$key}} : ($on{$key});
      ( $query_string .= "\&amp;$key=" . MkUrl($_) ) for @values;
     }
   }
 }
...
sub FormHead
 {
  ...
  if ($form{action} eq 'none') 
  ...
  else
   {
    ...
    ## use $on{changedfields} to store fielda changed by user
...
sub Input
 {
  ...
  if ($data{type} eq 'findable')
   {
    ...
    $data{show} ||= "<a style=\"font-size:11px; color:$clr{font_menu_title}; background-color:$clr{bg_menu_title}\"".
                    " href=\"$cgi_name?action=findshow;\" onclick=\"window.location.reload();\">".
                    "$lang{find}</a>";
    $data{containerstyle} = 'width:auto; white-space:nowrap;';
    $data{onchange} ="window.location = this.options[this.selectedIndex].value + '?action=findshow';";
   }
  ## TYPE:QUICKCREATOR ##
    ...
    if ( $data{data} =~ /^select.+from/i )
    ...
    elsif (ref($data{data}) eq 'HASH')
     {
      ...
      if ($data{order} eq 'byvalue')
       {
        foreach (sort keys %{$data{data}})
         {
          $sl_data[$cnt][0]   = $_;
          $sl_data[$cnt++][1] = ${$data{data}}{$_};
         }
       }
      else
       {
        foreach (sort { ${$data{data}}{$a} cmp ${$data{data}}{$b} } keys %{$data{data}})
         {
          $sl_data[$cnt][0]   = $_;
          $sl_data[$cnt++][1] = ${$data{data}}{$_};
         }
      ...
      for my $i (0 .. $#{$data{data}})
       {
        if( ref($data{data}[$i]) ne 'ARRAY' )
         {
          ## $data{data} it's a mono-dimensional array
          $sl_data[$i] = [$data{data}[$i], $data{data}[$i]];
         }
        else
         {
          ## $data{data} it's a bi-dimensional array so copy it
          $sl_data[$i] = $data{data}[$i];
         }
       }
     }
    else
     {
      ## it should be a simple text (type=>label)
      ## so do nothing
     }
   }
  ...
  if ($data{type} eq 'multiselect')
   {
    ...
    push @sl_data, ['all',$lang{all}] if $data{allvalue};

    for ( 0 .. $#sl_data )
     { 
      if ( $unescapedvalue =~ /$sl_data[$_][0]/ )
       { $dat2{$sl_data[$_][0]} = $sl_data[$_][1]; }
      else
       { push @dat1, [ $sl_data[$_][0], $sl_data[$_][1] ]; }
     }
  ...
  ## TYPE:FAXNUMBER ##
  elsif ($data{type} eq 'phonenumber' || $data{type} eq 'faxnumber')
   {
    ...
    $in_readonly_also = Img( src   => "$IG::img_url/$icon",
			     width => 16,
			     href  => $href,
...
END
    ...
    for my $i ( 0 .. $#sl_data )
     {
      next if    $_is_readonly
              && ($select{$data{name}}{$sl_data[$i][0]} ne ' selected'
              && !($i == 0 && !$unescapedvalue));

      my $label_value = MkEntities( $sl_data[$i][1] );
         $label_value =~ s/ {2}/ \&nbsp;/g;
              
      $field .= "\t<option value=\"".
                MkEntities( $sl_data[$i][0] ).
                "\" $select{$data{name}}{$sl_data[$i][0]}>".
                "$label_value</option>\n";
                
      $escapedvalue = MkEntities( $sl_data[$i][1] )
         if $select{$data{name}}{$sl_data[$i][0]} eq " selected";
     }
    $escapedvalue ||= MkEntities( $sl_data[0][1] );
    $field.="</select>\n";
   }
  ## TYPE:CONTACTFINDER ##
  ...
  ## TYPE:TEMPLATE ##
  elsif ( $data{type} eq 'template' && !$_is_readonly )
   {
    ...
    if ( ! $_is_readonly )
     {
      my $re = $cgi_name eq 'circular' || $cgi_name eq 'webmail'
             ? qr/html*|txt/
             : qr/html*|odt/;
  ...
  if (    $_is_readonly
       && $data{type} !~ /multiselect|select|basictable|sendmode|checkbox|radio|label/ )
   {
    ...
    $field = "<div style=\"white-space:normal; padding:1px; ".
                          "border:1px solid #999999; overflow:auto; ".
                          "background-color:#f4f4f4; $style\">".
	     "${escapedvalue}&nbsp;</div>";
    $field = HLayer( $field, $in_readonly_also ) if $in_readonly_also;
   } 
...
sub InputDocBox
 {
  ...
  $box .=   Input(	type  => 'radio',
	    ...
	    Input(	show=>$lang{copy_from_protocol},
			...
			style=>'font-size:10px;height:16px;width:100px;',
	    ...
	    Input(	show=>$lang{copy_from_server},
			labelstyle => 'border:0px; font-size:10px; '.
			              'width:auto; background:transparent',
        		name=>'builddoc_source5',
			style=>'font-size:10px; height:16px; width:180px;',
			onfocus=>"document.$form{name}.builddoc_choice[4].checked=true;",
			type=>'text',
			float => 'left' ).
			
            Img(        name      => 'addfromfile',
                        onclick   => "document.$form{name}.builddoc_choice[4].checked=true;".
                                     "winPopUp('filemanager?defact=2',".
                                     "550,500,'ProtocolFromServer');",
                        title     => $lang{copy_from_server},
...
sub Img
 {
  ...
  if ($data{href})
   { $html = "<a href=\"$data{href}\"$data{target}>$html$data{caption}</a>"; }
...
sub SysExec
 {
  ##XXX2DEVELOPE use String::ShellQuote
  ## Used by..
  ...
  my %data = @_;
  die("I need a command to execute!\n") if !$data{command};

  ## check command execution
  if ( ! -e $data{command} )
   {
    require IG::FileWhich;
    my @command     = split /\/|\\/, $data{command};
    my $_command    = pop( @command );
    my $_pcommand   = File::Which::which( $_command );
       $_pcommand ||= eval "`which $_command`";

    if ( $_command !~ /^(chmod|chown|perl)$/ )
     {
      $_pcommand eq $data{command}
      ? die("I can't execute '$data{command}' please check execute privileges\n")
      : die("I dont't have a right path to execute '$data{command}'. ".
            ( -e $_pcommand
              ? "Please try to modify igsuite.conf and fix this row: ".
                "\$ext_app{$_command} = '$_pcommand';"
              : "Please insert a right path to \$ext_app{$_command} inside ".
                "configuration file 'igsuite.conf'" ).
                "\n" );
     }
    $data{command} = $_pcommand;
  }
...
sub DbQuery
 {
  ...
  $qry =~ /^(insert|delete|select|update|copy|commit|begin|rollback|optimize)/i;
    ...
    if (!$conn{$dbase}{connection})
     {
      ...
      $result[$dbconn] = $conn{$dbase}{connection}->do("SET NAMES 'LATIN1'");
     }
...
sub MkLastNum
 {
  ...
  my $cid = DbQuery( query => "SELECT max($doc_info{$table}[1]) ".
                              "FROM $table ".
...
sub RelatedTo
 {
  ...
  my @related =
      ( ["contracts?action=proto&amp;docref=$id&amp;contactid=$contactid",
        ...
        ["binders?action=proto&amp;note=$id&amp;contactid=$contactid",
         $lang{binder} ],
        ["todo?action=protomaster&amp;todotext=$id&amp;btr=1&amp;contactid=$contactid",
         $lang{todo} ],
      );
  ...
  DbQuery("SELECT id, contactname, 'contracts', issue, docref, expire,".
	  ...
	  "FROM fax_sent where docref~*'$id' or note~*'$id' ".
          ...
          "UNION ".
          "SELECT todo.todoid, contacts.contactname, 'todo', todo.issue,".
          " '', todo.duedate, todo.owner, todo.todotext, '', 0 ".
          "FROM todo ".
          "LEFT JOIN contacts ON contacts.contactid = todo.contactid ".
          "WHERE todo.owner='$auth_user'".
          " and (todo.todotext~*'$id' or todo.description~*'$id') ".
  ...
  while ( my @row = FetchRow() )
   {
    ...
    $html .= TaskListItem( [ $row[2] eq 'To Do\'s'
                             ? ( $row[0], "todo?action=protomaster&amp;todoid=$row[0]")
                             : ParseLink($row[0]) ],
                           [ $row[1] ],
                           [ $row[2] ],
...
sub BuildDoc
 {
  ...
  if ( $data{choice} == 2 )
   ...
   {
    ## copy doc from the server
    die("You can't copy file from directory other than your Home.\n")
      if $on{builddoc_source5} !~ /^\/$IG::default_lang{home}\/$auth_user\//i;
    
    my $docfile = CkPath( $htdocs_dir . $S . $on{builddoc_source5} );
    my ($ext) = $docfile =~ /\.([^\.]+)$/;
    die("We need an extension! ".
        "check '$docfile' file or rename it.\n") if !$ext;
    ...
    IG::FileCopy($docfile,
		 ...
		 1);
    LogD('Copy document from server',
	 'update',
	 $data{type},
	 "$1.$2");
   }
    ...
    if ( $file_type eq 'application/x-zip' || $file_type eq 'application/msword' )
     {
      ...
      $file_type = 'application/msword'  if $file_doc =~ /\.(odt|sxw|doc|docx)$/i;
      $file_type = 'application/msexcel' if $file_doc =~ /\.(ods|xls|xlsx|xlt|sxc)$/i;
     }
 ...
 { 
  ...
  my %docs
   =(
     ...
     reports	  => ['report.gif',
		      'Report',
		      'reports?action=loadreport&amp;subact=execute&amp;id='],
     users	  => ['user.gif',
		      $lang{staff},
