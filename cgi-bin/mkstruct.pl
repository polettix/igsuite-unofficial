  ...
  if ( $IG::db_driver eq 'postgres' || !$IG::db_driver )
   {
	...
	$is_a_new_table++;
      ...
      for my $cnt ( 0 .. $db_tables_index{$table_name}{fields} )
       {
      ...
      if ( $is_a_new_table && $db_tables_index{$table_name}{oldname} )
       {
        writelog("- Populate new table '$table_name' ".
                 "with old values from '$db_tables_index{$table_name}{oldname}'\n");
        $result = $conn->exec("insert into $table_name".
                              " ( select * from $db_tables_index{$table_name}{oldname} )");
       }

      ## Create index
      if ( $db_tables_index{$table_name}{indexes} )
       {
        my $cnt = 0;
        for ( split (/\;/, $db_tables_index{$table_name}{indexes}) )
         {
          $cnt++;
      ...
      if ( $is_a_new_table && $db_tables_index{$table_name}{oldname} )
       {
        writelog("- Populate new table '$table_name' ".
                 "with old values from '$db_tables_index{$table_name}{oldname}'\n");
        my $result = $dbh->do("insert into $table_name".
                              " select * from $db_tables_index{$table_name}{oldname}");
       }
