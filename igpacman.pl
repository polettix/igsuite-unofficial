...
my @parameters = ## parameters handled by this script
    ( [ 'cgi_dir',
      ...
      [ 'db_port',
        'Database server port (mysql:3306; postgres:5432; sqlite:none)']
   );
...
sub default_action
 {
  ...
  print "<div style=\"padding:5px; border:2px solid #999999; font-size:11px;".
        ...
        "http://www.cpan.org".
        ( $OS eq 'UNIX'
          ? " or try to execute <a href=\"http://www.igsuite.org/install_modules.pl\">".
            "install_modules.pl</a> before this script."
...
sub ck_values
 {
  ...
  $errors += _ck_result( (! -d $htdocs_dir || ! -w $htdocs_dir),
                        'htdocs_dir',
                        "Is not a directory or is not writable by $www_user",
                        "OK! <a href=\"javascript:winPopUp(".
                                      "'igpacman.pl?action=htdocs_test'".
                                      ",800,500,'test')\">( Test Me! )</a>" );
  ...
  $errors += _ck_result( ( $db_driver ne 'sqlite' && $db_port !~ /^[0-9]+$/),
                        'db_port',
...
sub preload_package
 {
  ...
  print "<table cellpadding=13 style=\"text-align:left;".
        " width:750px; color:#000000; background:#DDDDDD;\">";

  ## form field to embedded choiche
  print "<tr>".
        "<td style=\"vertical-align:top;\">".
        "<input type=\"radio\" name=\"source_type\" value=\"embedded\" checked></td>".
        "<td>Use the embedded package</td>".
        "<td valign=\"top\">IGSuite 4.0 (latest release)</td>".
        "</tr>" if defined( &_get_igsuite_zip );

  ## form field to internet choiche
  print "<tr>".
        "<td style=\"vertical-align:top;\">".
        "<input type=\"radio\" name=\"source_type\" value=\"internet\"".
        ( defined(&_get_igsuite_zip) ? '' : ' checked' ) . "></td>".
        "<td>URL from which to download IGSuite package in ".
        ...
        "Default: <i>http://downloads.sourceforge.net/isogest/igsuite-4.0.1".
        "$package_file_format</i></div></td>". 
        "</tr>" if _is_internet_alive();

  ## form field to local choiche
  print "<tr>".
        "<td style=\"vertical-align:top;\">".
        "<input type=\"radio\" name=\"source_type\" value=\"local\"></td>".
        "<td>Copy and unpack a local (server side) IGSuite package in ".
        "<strong>$package_file_format format</strong></td>".
        "<td valign=\"top\">",
        textfield( -name      => 'package_path',
                   ...
                   -maxlength => 200 ),
        "<div style=\"font-size:10px\">".
        "Example: <i>" . ( $OS eq 'UNIX'
                           ?  '/tmp/igsuite-4.0.1'.$package_file_format
                           :  'c:\\Temp\\igsuite-4.0.1'.$package_file_format ).
        "</i></div></td>".
        "</tr>";

  if ( $OS eq 'UNIX' && ! $_is_archive_zip_installed )
   {
    print "<tr><td style=\"margin-top:50px\"></td>".
          "<td style=\"margin-top:50px\">".
          "Unpack application (we need 'tar')</td>".
          "<td style=\"margin-top:50px\">".
  ...
  print "<tr><td colspan=3 style=\"text-align:center; color:#ff3300; font-weight:bold\">".
        "&gt; Depend of your choice, this step may take a while! Be patient! &lt;</td></tr></table>";
  ...
  if (    $OS eq 'UNIX'
     ...
     )
   {
    push @err_msg, "Missing or invalid unpack application please ".
                   "insert a right path to 'tar' application or ".
                   "install Archive::Zip Perl module.";
    preload_package();
    return;
   }
  ...
  if ( -e $target_file )
   {
    unlink( $target_file )
      or die("Can't delete previous old package '$target_file'. ".
             "Please do it manually! And try again.\n");
   }

  if ( param('source_type') eq 'embedded' )
   {
    ## retrieve the package embedded in this file
    if ( $_is_archive_zip_installed )
     { _get_igsuite_zip( $target_file ); }
    else
     { _get_igsuite_tar( $target_file ); }
   }
  elsif ( param('source_type') eq 'local' )
   {
    ...
    if ( -e $package_path && $package_path =~ /\.(tar\.gz|zip)$/ )
     ...
     {
      push @err_msg, "You specified an invalid file path.<br>Please be sure ".
...
sub install_end
 {
  ...
  my $install_success =   !$install_status 
                        || $install_status =~ /Error|ABORT|ALERT|ATTENTION|syntax error/m
                      ? 0
                      : 1;
...
END
  ...
  print "Look below at installation logs...<br><br>".
        ...
        ( $install_success
          ? button( -name    => 'updateb',
                    -style   => 'margin:20px 3px 0px 3px; float:left;',
                    -onclick => "javascript:winPopUp('igsuited?action=--update-igsuite',300,150,'update');",
                    -value   => 'Update IGSuite with last patches')
          : ''),
...
sub _some_system_check
 {
  ...
  die( "Can't execute IGPacMan under mod_perl. Remember, after installing ".
       "IGSuite if you want to use mod_perl you will have to configure ".
       "ModPerl::PerlRun instead of ModPerl::Regisrty.\n")
  ...
  if ( $db_driver eq 'postgres' )
   {
    ...
    _ck_result( _cmp_eq( Pg->PGRES_CONNECTION_OK, $conn->status ) == 0,
                ...
                " - that Postgres is running on server '$db_host' ".
                  " on port '$db_port' and can accept remote connection;<br>\n".
...
sub Footer
 {
  ...
  if ( @err_msg )
   {
    print "<div style=\"margin-top:30px;clear:both;border: 2px solid #FFFFFF;".
...
sub _get_iglogo
 {
  my $logo_img = 
q{M1TE&.#EA%@`3`.>/``<4-P<;1PD@6P<B7`DC8`\\D4A\\G-0LI?0LL@@TL?0LM
