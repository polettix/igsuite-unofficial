...
sub mk_install
 {
  ...
  my @procs =qw(soc_fax		soc_name	soc_email	soc_address
		...
		soc_country	soc_logo	soc_site	soc_contactid
		...
		db_port		db_name         debug           users_allow
  ...
  ## Copy original igsuite.conf to igsuite.conf.orig
  {
   ...
   ## check File::Which for later use
   eval 'require File::Wich';
   my $_file_wich = !$@ ? 1 : 0;
     ...
     for ( @procs )
      {
       ...
       if ( $_file_wich && /^ext\_app\{(.+)\}$/ && !$value )
        {
         ## try to discover executable paths
         my $executable = $1;
         $value = File::Which::wich( $executable );
        }
